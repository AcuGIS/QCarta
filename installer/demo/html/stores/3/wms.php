<?php
	...
	$url = 'http://localhost/cgi-bin/qgis_mapserv.fcgi?VERSION=1.1.0&map='.QGIS_FILENAME_ENCODED.'&SERVICE=WMS&'.$_SERVER['QUERY_STRING'];
	if($_GET['request'] == 'GetCapabilities') {
       ...
       echo str_replace('://localhost/cgi-bin/qgis_mapserv.fcgi?', '://'.$_SERVER['HTTP_HOST'].'/stores/'.LAYER_ID.'/'.strtolower('WMS').'.php?'.$acc_key, $content);
	}else{
	   readfile($url);
	}
?>
