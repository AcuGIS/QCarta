...
function install_qcarta_tiles(){
	
	apt-get -y install golang-go
	
	mkdir -p "${DATA_DIR}/qcarta-tiles/cache_data"
	chown -R www-data:www-data ${DATA_DIR}/qcarta-tiles
	
	QCARTA_CACHE_PURGE_TOKEN=$(< /dev/urandom tr -dc A-Za-z0-9 | head -c66);
	
	sed -i.save "
s|/var/www/data|$DATA_DIR|
s/QCARTA_CACHE_PURGE_TOKEN_VALUE/${QCARTA_CACHE_PURGE_TOKEN}/" installer/qcarta-tiles/qcarta-tiles.service
	
	sed -i.save "s|/var/www/data|$DATA_DIR|" installer/qcarta-tiles/main.go
	
	pushd installer/qcarta-tiles
	    go build -o qcarta-tiles
		
		mv qcarta-tiles /usr/local/bin/
		chmod +x /usr/local/bin/qcarta-tiles
		
		mv qcarta-tiles.service /etc/systemd/system/qcarta-tiles.service
	popd
	
	
	chmod +x /etc/systemd/system/qcarta-tiles.service
	systemctl daemon-reload

	systemctl enable qcarta-tiles
	systemctl start qcarta-tiles
...
mkdir -p "${WWW_DIR}/geostories"

cp installer/usdemo.qgs "${DATA_DIR}/stores"
cp installer/settings.json ${DATA_DIR}/settings.json
...
www-data ALL = NOPASSWD: /usr/local/bin/qcarta-tiles_ctl.sh
CAT_EOF
