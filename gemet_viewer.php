<?php
  session_start();
	...
	$obj	= new topic_Class($database->getConn(), $_SESSION[SESS_USR_KEY]->id, 'gemet');
	$topics  = $obj->getRows();
	
    $qgslay_obj = new qgs_layer_Class($database->getConn(), SUPER_ADMIN_ID);
    $story_obj  = new geostory_Class($database->getConn(), SUPER_ADMIN_ID);
    $link_obj = new web_link_Class($database->getConn(), SUPER_ADMIN_ID);
    $doc_obj = new doc_Class($database->getConn(), SUPER_ADMIN_ID);
?>
<!doctype html>
<html lang="en">
  <head>
      ...
      <div class="py-8">
        <div class="container mx-auto px-5 max-w-7xl">
            ...
            ?>
                ...
                <!-- Layers -->
                <?php $layers = $qgslay_obj->getByIds($typeIds['layer']);
                  ...
                  <div class="group">
                    <a href="layers/<?=$row->id?>/index.php" class="block bg-white border border-gray-200 rounded-lg overflow-hidden hover:border-blue-500 transition-colors duration-200" target="_blank">
                      <div class="relative">
                        <img src="<?=$image?>" alt="<?= str_replace('_', ' ', $row->name) ?>" 
                ...
                <!-- Stories -->
                <?php $stories = $story_obj->getByIds($typeIds['geostory']);
                    while($row = pg_fetch_object($stories)) {
                ...
                <!-- Links -->
                <?php $links = $link_obj->getByIds($typeIds['web_link']);
                    while($row = pg_fetch_object($links)) {
                ...
                <!-- Docs -->
                <?php $docs = $doc_obj->getByIds($typeIds['doc']);
                while($row = pg_fetch_object($docs)) {
