...
function tabbedPopup(e){
  ...
  visibleLayers.forEach((layer, index) => {
    ...
    // Strip prefix from layer name for WMS requests (QGIS expects plain names)
    const plainLayerName = stripLayerPrefix(layer.name);
    if(u in urlNames){
      urlNames[u].push(plainLayerName); // append name to array
    }else{
...
layerConfigs.forEach((cfg, idx) => {
  // Create WMS layer using /mproxy/service for caching
  // WMS_SVC_URL should already be /mproxy/service?map=... from getMproxyBaseUrl()
  // Strip any prefix from layer name for WMS requests (QGIS expects plain names)
