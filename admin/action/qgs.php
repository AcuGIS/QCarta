...
function get_name_from_source($sources){
    if(count($sources) == 1){
        return basename($sources[0]);
    }else{
        foreach($sources as $src){
            if(str_ends_with($src, '.qgs') || str_ends_with($src, '.qgz') || str_ends_with($src, '.zip')){
                return basename($src);
            }
        }
        return 'store name';
...
function create_chart_json($data_dir){
    $chart_configs = [];
    ...
    foreach ($xml_files as $xml_file) {
		...
		if ($xml_content) {
			$base_title = (string)$xml_content->xpath('//Option[@name="plot_layout"]/Option[@name="title"]/@value')[0] ?? '';
			$filename = basename($xml_file, '.xml');
			$title = $base_title ? "$filename - $base_title" : $filename;
			$chart_configs[] = array(
				'filename' => urlencode(basename($xml_file)),
				'title' => $title
			);
		}
	}
	
	$js = json_encode($chart_configs, JSON_UNESCAPED_SLASHES);
	file_put_contents($data_dir.'/charts.json', $js);
}
    ...
    if(isset($_SESSION[SESS_USR_KEY]) && $_SESSION[SESS_USR_KEY]->accesslevel == 'Admin') {
            ...
            $newId = 0;
						...
						if(isset($_POST['autofill'])){
						    switch($_POST['store_source']){
								case 1: $_POST['name'] = get_name_from_source($_POST['source']);            break;
								case 2: $_POST['name'] = basename($_POST['src_url']); break;
								case 3: $_POST['name'] = $_POST['qf_name'];           break;
								default:$_POST['name'] = 'store name';                break;
							}
							$acc_obj = new access_group_Class($database->getConn(), $_SESSION[SESS_USR_KEY]->id);
							$usr_grps = $acc_obj->getByKV('user', $_SESSION[SESS_USR_KEY]->id);
							$_POST['group_id'] = array_keys($usr_grps);
						}
	              ...
	              $newId = $obj->create($_POST);
							...
							if($newId > 0){
								...
								$result = ['success' => true, 'message' => 'Store successfully created!', 'id' => $newId, 'name' => $_POST['name']];
					...
					if($qgis_file !== false){
						...
						$kv = ['Projection' => (string) $projection, 'BoundingBox' => $bounding_box, 'Layouts' => implode(',', $layout_names), 'Layers' => implode(',', $layer_names),
							...
							'OpenLayers' => "<a href=".$base_url.'/wms?REQUEST=GetProjectSettings'." target='_blank'>".$base_url.'/wms?REQUEST=GetProjectSettings'."</a>"
						];
