...
function install_store($id, $post, $conn){
	...
	if(isset($post['qf_name'])){
		// if no plugin or project dir
		if(!$with_qfield || !is_dir(QF_HOME.'/projects/'.$post['qf_name'])){
			return false;
		}
		// link store data_dir to QField project dir
		if(!symlink(QF_HOME.'/projects/'.$post['qf_name'], $data_dir) || !mkdir($html_dir)){
			return false;
		}
		$post['source'] = array();	//no source files, we just link
		
	}else if(!mkdir($data_dir) || !mkdir($html_dir)){
		return false;
	}
            ...
            $newId = 0;
	              ...
	              $newId = $obj->create($_POST);
					...
					if(count($ref_ids) > 0){
						...
						if(is_link($data_dir)){	// if link to QField project
							unlink($data_dir);		// remove only link
						}else{
							rrmdir($data_dir);
						}
						rrmdir($html_dir);
