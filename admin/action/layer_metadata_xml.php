...
if($layer['proxyfied'] == 't') {
	require_once(__DIR__ . '/../../inc/mproxy.php');
	try {
		$base_url = getMproxyBaseUrl($id); // Use layer_id, function will resolve to store_id
		$svc_url .= $base_url;
	} catch (Exception $e) {
		error_log("admin/action/layer_metadata_xml.php: Failed to get mproxy base URL for layer_id $id: " . $e->getMessage());
		$svc_url .= '/mproxy/service'; // Fallback
	}
} else {
	$svc_url .= '/layers/'.$id.'/proxy_qgis.php';
}
