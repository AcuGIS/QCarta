...
RUN apt-get -y update && \
	apt-get -y --no-install-suggests --no-install-recommends install apache2 libapache2-mod-php php-pgsql php-mbstring php-xml php-zip php-yaml gdal-bin postgresql-client mapproxy && \
	apt-get -y clean all && \
...
# copy web files
COPY --chown=www-data:www-data index.php login.php logout.php about.php contact.php proxy_wfs.php public.php viewer.php /var/www/html/
...
COPY docker/mapproxy_seed_ctl.sh /usr/local/bin/mapproxy_seed_ctl.sh
RUN chmod +x /usr/local/bin/mapproxy_seed_ctl.sh && \
